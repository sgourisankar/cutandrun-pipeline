...
rule make_archive:
    input:
        ...
        expand("SM/macs2/{sample}.paired.q{qv}."+macs2_type+"/{sample}_summits.bed",qv=macs2_q,allow_missing=True),
...
rule align:
    input:
        read1=fastq_file_dir+"/"+fastq_prefix+"{sample}"+fastq_mid+"R1"+fastq_suffix+".fastq.gz",
        read2=fastq_file_dir+"/"+fastq_prefix+"{sample}"+fastq_mid+"R2"+fastq_suffix+".fastq.gz"
    output:
        sam="SM/sambam/{sample}.sam",
    ...
    shell:
        ...
        bowtie2 {params} -1 {input.read1} -2 {input.read2} -S {output.sam} --met-file {output.met}
...
rule process_bam:
    ...
    shell:
        ...
        bamCoverage --bam {input.bam} {params.bc} -o {output.norm_bw} 
        '''
...
rule macs2_1of2:
    ...
    output:
        ...
        summits="SM/macs2/{sample}.paired.q{qv}."+macs2_type+"/{sample}_summits.bed",
